...
router.get("/dashboard/stats", authenticate, async (req, res) => {
  ...
  const inventory = await db.select().from(inventoryTable).where(eq(inventoryTable.tenantId, tenantId));
  ...
  const totalSpending = pos.filter(p => p.status !== "draft" && p.status !== "cancelled")
    .reduce((sum, p) => sum + parseFloat(p.totalAmount ?? "0"), 0);
  const inventoryValue = inventory.reduce((sum, i) =>
    sum + parseFloat(i.quantity ?? "0") * parseFloat(i.unitCost ?? "0"), 0);
  const lowStockCount = inventory.filter(i =>
    parseFloat(i.quantity ?? "0") <= parseFloat(i.reorderLevel ?? "0")).length;
...
router.get("/dashboard/spending-by-vendor", authenticate, async (req, res) => {
  const tenantId = (req as any).user.tenantId;
  const vendors = await db.select().from(vendorsTable).where(eq(vendorsTable.tenantId, tenantId));
  const pos = await db.select().from(purchaseOrdersTable).where(eq(purchaseOrdersTable.tenantId, tenantId));

  const spending = vendors.map(vendor => {
    const vendorPOs = pos.filter(p => p.vendorId === vendor.id && !["draft", "cancelled"].includes(p.status));
  ...
  return res.json(spending.filter(s => s.totalSpent > 0).sort((a, b) => b.totalSpent - a.totalSpent));
});

router.get("/dashboard/orders-by-status", authenticate, async (req, res) => {
  const tenantId = (req as any).user.tenantId;
  const pos = await db.select().from(purchaseOrdersTable).where(eq(purchaseOrdersTable.tenantId, tenantId));
  ...
  return res.json(Object.entries(statusMap).map(([status, count]) => ({ status, count })));
});
